...
Forward simulator for tree-structured mutational signature data: the
switch-plus-drift process described in simulator_spec.md.

This is the data-generating process for simulation studies of the Tree-HDP
inference model. Signatures are drawn from a named catalogue, never
synthesised, and the ground truth is not generated from the inference model
(simulator_spec.md section "The two invariants"). The module takes plain
dicts and dataclasses only; it has no dependency on `src/config.py` or YAML
-- parsing a config file into the dict this module consumes is
`generate_data.py`'s job.
...
class TreeSwitchDriftGenerator:
    ...
    config : dict
        ...
        config-layer concerns handled by `generate_data.py`). Required
        top-level keys: `repertoire`, `switching`, `levels`, `forest`,
        `burden`, `counts`.
    seed : int, optional
    ...
    def get_mutation_count_matrix(self) -> pd.DataFrame:
        ...
        idempotent (see the NQ6 fix in this file's git history)."""
