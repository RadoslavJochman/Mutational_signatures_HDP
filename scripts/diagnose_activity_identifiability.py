...
def sumzero_singular(S):
    ...
    B = UB[:, : K - 1]        # orthonormal basis of the sum-zero subspace (P has K-1 unit eigenvalues)
    M = B.T @ S               # S in that basis; singular values of M are the activity-direction gains
...
def soft_energy_fraction(vec, U):
    ...
    if energy.sum() <= 0:
        return np.nan
    half = len(energy) // 2        # bottom half = softest dirs; isotropic baseline ~0.5 (exact only for even K-1)
...
def loglik_gap(counts, p_A, p_B, eps=1e-12):
    ...
    pA = np.clip(p_A, eps, None); pB = np.clip(p_B, eps, None)
...
def directionality_summary(dLL_signed, dL1, eps_d=0.5, eps_t=0.02):
    ...
    out = {}
    g = np.asarray(dLL_signed, float); g = g[~np.isnan(g)]
    if g.size:
        nA, nB = int((g > eps_d).sum()), int((g < -eps_d).sum())
        dec = nA + nB
        out.update(data_favA=nA, data_favB=nB,
                   data_tied=int((np.abs(g) <= eps_d).sum()),
                   net_dLL=float(g.sum()),
                   consistency=(max(nA, nB) / dec) if dec else np.nan)
    dl = np.asarray(dL1, float); dl = dl[~np.isnan(dl)]
    if dl.size:
        out.update(truth_favA=int((dl > eps_t).sum()),
    ...
    out["agreement"] = (float((np.sign(gg[m]) == np.sign(dd[m])).mean())
...
def main():
    ...
    ap.add_argument("--per-camp-sigs", action="store_true",
    ...
    e_al = [np.stack([post[ev].values[c][:, :, perms[c]]
                      for c in range(n_chains)]) for ev in e_vars]
    chain_act = np.zeros((n_chains, K)); tot = 0
    for a in e_al:
        chain_act += a.mean(axis=1).sum(axis=1); tot += a.shape[2]
    chain_act /= max(tot, 1)
    _camps = detect_camps(chain_act)
    campA, campB, kstar = _camps["campA"], _camps["campB"], _camps["kstar"]

    S_by_chain = np.stack([S[c][:, perms[c], :].mean(axis=0)
    ...
    if args.newick and (args.true_acts or args.counts):
        try:
            dr = nodes_by_depth(
                build_forest(Path(args.newick).read_text().strip()))
        except Exception as exc:
            print(f"(node map failed -- phylox/newick? {exc})")
    truth = (pd.read_csv(args.true_acts, index_col=0)
             if args.true_acts else None)
    ...
    for ev, a in zip(e_vars, e_al):
        ...
        for node in range(a.shape[2]):
            ...
            if args.per_camp_sigs:
                ...
                pA = (eA @ true_S); pB = (eB @ true_S)
            pA = pA / pA.sum(); pB = pB / pB.sum()
            rec = {
                "node": label if label is not None else f"{depth}:{node}",
                "depth": depth, "de_l1": de_l1,
                "dp_l2": float(np.linalg.norm(dp)),
                "tv": tv_distance(pA, pB),
                "js": js_divergence(pA, pB),
                "ratio_dp_de": (float(np.linalg.norm(dp) /
                                (np.linalg.norm(de) + 1e-12))),
                "de_soft_frac": soft_energy_fraction(de, U),
            }

            te = (truth.loc[label].values
                  if (truth is not None and label in
                      (truth.index if truth is not None else [])) else None)
            p_true = None
            if te is not None:
                p_true = te @ true_S; p_true = p_true / p_true.sum()

            if counts is not None and label in (counts.index if counts is not None else []):
            ...
            if p_true is not None:
                ...
                kls = [kl_div(p_true, best[c, node] @ S_by_chain[c])
    ...
    if len(split) and (have_data or have_truth):
        ...
        D = directionality_summary(
            split["dLL_signed"].values if have_data
            else np.full(len(split), np.nan),
            split["dL1_AminusB"].values if have_truth
            else np.full(len(split), np.nan),
            eps_d, eps_t)
        for key in ("data_favA", "data_favB", "data_tied", "net_dLL",
                    "consistency", "truth_favA", "truth_favB", "truth_tied",
                    "n_both", "agreement"):
            if key in D:
    ...
    if len(split) and {"kl_true_A", "kl_true_B"} <= set(split.columns):
        if {"dll_A_vs_true", "dll_B_vs_true"} <= set(split.columns):
            ...
            summary["dll_best_vs_true_median"] = float(
                split[["dll_A_vs_true", "dll_B_vs_true"]]
                .max(axis=1).dropna().median())
        ...
        summary["frac_nodes_B_closer"] = (
            ...
            if len(bothkl) else np.nan)
        if "kl_true_bestchain" in split.columns:
            summary["kl_true_bestchain_median"] = float(
                split["kl_true_bestchain"].dropna().median())

    pd.DataFrame([summary]).to_csv(
        outdir / "activity_ident_summary.csv", index=False)
    print(f"Written: {outdir/'activity_ident_by_node.csv'}, "
