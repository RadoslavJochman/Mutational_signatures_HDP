...
def main():
    ...
    ap.add_argument(
        "--metric",
        default="hellinger",
        choices=sorted(DISTRIBUTION_METRICS),
    ...
    sig_d = np.stack(
        [
            [metric_fn(Smean_al[c, k], true_S[k]) for c in range(n_chains)]
            for k in range(K)
        ]
    )  # (K, chains)
    ...
    pd.DataFrame(
        {
            ...
            "between_camp": between,
            "between_camp_cos": between_cos,
            "metric": a.metric,
    ...
    pd.DataFrame({"chain": np.arange(n_chains), "camp": memb}).to_csv(
        out / "mode_camps.csv", index=False
    )
    ...
    pd.DataFrame(sig_rows, columns=["signature", "chain_i", "chain_j", "value"]).to_csv(
        out / "mode_chain_signature.csv", index=False
    )
    pd.DataFrame(act_rows, columns=["signature", "chain_i", "chain_j", "value"]).to_csv(
        out / "mode_chain_activity.csv", index=False
    )

    print(
        f"camps: A={A} B={B}; most spectrum-divergent signature {kdiv} "
