...
def main():
    ...
    ap.add_argument("--signatures", type=int, nargs="+", default=None,
    ...
    ap.add_argument("--show-cosine", action="store_true",
    ...
    metric = args.metric or (summary["metric"].iloc[0]
                             if "metric" in summary.columns else "cosine")
    metric_fn, higher_is_better = DISTRIBUTION_METRICS[metric]
    metric_label = {"hellinger": "Hellinger to truth", "tv": "total variation to truth",
                    "jensen_shannon": "Jensen-Shannon to truth",
                    "cosine": "cosine to truth"}.get(metric, f"{metric} to truth")
    metric_short = {"hellinger": "Hellinger", "tv": "TV", "cosine": "cosine",
                    "jensen_shannon": "JS"}.get(metric, metric)
    ...
    act_spread = (summary.set_index("signature")["act_spread"].reindex(range(K)).values
                  if "act_spread" in summary.columns else None)
    ...
    ax.set_xticklabels([f"{order[j]}\n{exposure[order[j]]:.1f}" for j in range(K)],
                       fontsize=8)
    ...
    fig.tight_layout(); save(fig, outdir, "fig_recovery_heatmap")
    plt.close(fig)
    ...
    fig, axes = plt.subplots(1, len(ks), figsize=(4.9 * len(ks), 3.0),
                             squeeze=False, sharey=True)
    for ax, k in zip(axes[0], ks):
        for cid, idx, col, base in [("A", A, P["stiff"], 1.0), ("B", B, P["soft"], 0.0)]:
            if idx:
                y = base + rng.uniform(-0.12, 0.12, size=len(idx))
                ax.scatter(M[idx, k], y, s=80, color=col, edgecolor="black",
                           lw=0.5, label=f"cluster {cid}", zorder=3)
                for c, yc in zip(idx, y):
                    ax.annotate(str(c), (M[c, k], yc), fontsize=7, ha="center",
                                va="center")
        ax.set_yticks([0.0, 1.0]); ax.set_yticklabels(["cluster B", "cluster A"])
        ax.set_ylim(-0.5, 1.5)
    ...
    fig.tight_layout(); save(fig, outdir, f"fig_mode_separation{suffix}")
    plt.close(fig)
    ...
    fig, axes = plt.subplots(1, 2 if two else 1, figsize=(9.6 if two else 5.4, 4.0),
                             squeeze=False)

    def _spread_panel(ax, vals, ylabel, color):
        ax.scatter(exposure, vals, s=60, color=color, edgecolor="black", lw=0.5,
                   zorder=3)
        for k in range(K):
            ax.annotate(str(k), (exposure[k], vals[k]), fontsize=8,
                        xytext=(4, 3), textcoords="offset points")
        ax.set_xscale("log")
        ax.set_xlabel("true signature exposure (log scale)")
        ax.set_ylabel(ylabel)
        ax.grid(color=P["grey"], lw=0.6, alpha=0.35); ax.set_axisbelow(True)

    _spread_panel(axes[0][0], spread,
                  f"across-chain spread of signature {metric_short.lower()}",
                  P["accent"])
    axes[0][0].set_title("Signatures")
    if two:
        _spread_panel(axes[0][1], act_spread,
                      "relative across-chain spread of activity", P["soft"])
        axes[0][1].set_title("Activities")
    else:
        axes[0][0].set_title("Disagreement concentrates at low exposure")
    fig.tight_layout(); save(fig, outdir, "fig_spread_vs_exposure")
    plt.close(fig)
    ...
    def _head(k):
        ...
        return (f"Signature {k} ({inside}exposure {exposure[k]:.1f}), "
    ...
    def _spectrum_panel(ax, k):
        ...
        ax.bar(ch, sp["truth"].values, width=0.85, color=P["grey"], alpha=0.5,
               label="truth", zorder=2)
        ax.scatter(ch, sp["campA"].values, s=12, color=P["stiff"], zorder=4,
                   label=_lab("cluster A", sp["campA"], sp["truth"]))
        ax.scatter(ch, sp["campB"].values, s=12, color=P["soft"], zorder=3,
                   label=_lab("cluster B", sp["campB"], sp["truth"]))
        ax.set_xlabel("mutation channel"); ax.set_ylabel("probability")
    ...
    fig.tight_layout(); save(fig, outdir, f"fig_split_signature{suffix}")
    plt.close(fig)
    n_fig = 4
    ...
    if act_path.exists():
        ...
        if wide:
            ...
            if args.signatures is not None:
                print("note: mode_activity.csv is the legacy single-signature form; "
                      "re-run diagnose_modes.py for activity on other signatures")
        else:
            ks_act = ks
        fig, axes = plt.subplots(1, len(ks_act), figsize=(4.6 * len(ks_act), 4.4),
                                 squeeze=False)
        for ax, k in zip(axes[0], ks_act):
            ...
            ax.scatter(d["campA"], d["campB"], s=42, color=P["accent"],
            ...
            t = _tag(k); lead = f"Signature {k} ({t})" if t else f"Signature {k}"
            ax.set_title(f"{lead}: total A {d['campA'].sum():.1f}, "
                         f"B {d['campB'].sum():.1f}")
        fig.tight_layout(); save(fig, outdir, f"fig_activity_split{suffix}")
        plt.close(fig)
        n_fig += 1
    ...
    def _chain_fig(csv_path, fig_name, good_low, cbar_label):
        ...
        if not long and args.signatures is not None:
            print(f"note: {csv_path.name} is the legacy single-signature form; "
        ...
        mid = 0.5 * (vmn + vmx)
        fig, axes = plt.subplots(1, len(ks_cc),
                                 figsize=((0.5 * n_chains + 1.8) * len(ks_cc),
                                          0.5 * n_chains + 1.8), squeeze=False)
        im = None
        for ax, k in zip(axes[0], ks_cc):
            ...
            for i in range(n_chains):
                for j in range(n_chains):
                    ...
                    ax.text(j, i, f"{ccm[i, j]:.3f}", ha="center", va="center",
    ...
    n_fig += _chain_fig(sig_csv, "fig_chain_signature", not higher_is_better,
                        f"between-chain {metric_short.lower()}")
    n_fig += _chain_fig(indir / "mode_chain_activity.csv", "fig_chain_activity",
                        True, "between-chain activity distance (Bray-Curtis)")
    ...
    def _draw_chain(fig, ax, csv_path, k, good_low, title, vmax=None):
        ccdf = pd.read_csv(csv_path)
        vcol = "value" if "value" in ccdf.columns else "cosine"
        m = ccdf[ccdf["signature"] == k].pivot(index="chain_i", columns="chain_j",
              values=vcol).values[np.ix_(chain_order, chain_order)]
        if good_low:
            vmn, vmx = 0.0, (vmax if vmax is not None else float(m.max()))
        else:
            vmn, vmx = float(m.min()), 1.0
        mid = 0.5 * (vmn + vmx); cmap = "YlGnBu_r" if good_low else "YlGnBu"
        im = ax.imshow(m, cmap=cmap, vmin=vmn, vmax=vmx)
        ax.set_xticks(range(n_chains)); ax.set_yticks(range(n_chains))
        ax.set_xticklabels([f"ch{c}" for c in chain_order], rotation=90, fontsize=7)
        ax.set_yticklabels([f"ch{c} ({camp[c]})" for c in chain_order], fontsize=7)
        for i in range(n_chains):
            for j in range(n_chains):
    ...
    if sig_csv.exists() and act_csv.exists():
        ...
        def _colmax(csv_path):
            d = pd.read_csv(csv_path); vc = "value" if "value" in d.columns else "cosine"
        ...
        fig, axes = plt.subplots(len(rows), 2,
                                 figsize=(2 * (0.5 * n_chains + 1.9),
                                          len(rows) * (0.5 * n_chains + 1.5)),
                                 squeeze=False)
        for r, k in enumerate(rows):
            _draw_chain(fig, axes[r][0], sig_csv, k, not higher_is_better,
                        f"signature {k}, signature", vmax=sig_vmax)
            _draw_chain(fig, axes[r][1], act_csv, k, True,
                        f"signature {k}, activity", vmax=act_vmax)
        fig.tight_layout(); save(fig, outdir, f"fig_chain_split{suffix}")
        plt.close(fig); n_fig += 1
