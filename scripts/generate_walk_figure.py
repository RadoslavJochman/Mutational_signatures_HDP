...
def main():
    ...
    def draw_pie(ax, center, vec, radius=0.05):
        ...
        for i, frac in enumerate(vec):
            if frac <= 0:
                continue
            theta = frac * 360.0
            ax.add_patch(Wedge((cx, cy), radius, start - theta, start,
                               facecolor=sig_colors[i], edgecolor="white",
                               linewidth=0.6, zorder=3))
            start -= theta
        ax.add_patch(plt.Circle((cx, cy), radius, fill=False,
                                edgecolor="dimgray", linewidth=0.5, zorder=4))

    fig = plt.figure(figsize=(12.0, 4.6))
    gs = fig.add_gridspec(1, 2, width_ratios=[1.7, 1.0], wspace=0.10)

    axL = fig.add_subplot(gs[0, 0])
    axL.set_xlim(0, 1.5); axL.set_ylim(0, 1); axL.set_aspect("equal")
    axL.axis("off")
    ...
    ex, ey = pos[E0]
    axL.text(ex, ey - 0.095, r"$e_0$", fontsize=12, color="black",
             ha="center", va="top")
    ...
    for k in (-1, 0, 1):
        axL.add_patch(plt.Circle((gap_x + k * 0.05, 0.42), 0.011,
                                 color="gray", zorder=2))
    ...
    lx, ly = pos[leaf]
    axL.text(lx, ly - 0.095, r"$j$", fontsize=13, color="black",
             ha="center", va="top")
    con = FancyArrowPatch((lx + 0.05, ly), (lx + 0.47, ly),
                          transform=axL.transData, mutation_scale=12,
    ...
    for i, col in enumerate(sig_colors):
        cy = 0.96 - i * 0.06
        axL.add_patch(plt.Rectangle((0.02, cy - 0.014), 0.022, 0.028,
                                    facecolor=col, edgecolor="none",
                                    transform=axL.transAxes, clip_on=False))
        axL.text(0.052, cy, f"Sig {i+1}", fontsize=8.5, color="dimgray",
                 transform=axL.transAxes, va="center", ha="left")

    # spectrum panel
    axR = fig.add_subplot(gs[0, 1])
    spec = activities[leaf] @ S
    axR.bar(np.arange(96), spec, color=np.repeat(spec_block_colors, 16), width=0.85)
    axR.set_xlim(-1, 96); axR.set_ylim(0, spec.max() * 1.18)
    axR.set_xticks([]); axR.set_yticks([])
    for s in ["top", "right", "left"]:
        axR.spines[s].set_visible(False)
    axR.spines["bottom"].set_color("gray")
    axR.set_xlabel("96 mutation channels", fontsize=10, color="dimgray")
    axR.set_title(r"$e_j\,S$  (mixed spectrum of node $j$)", fontsize=11,
                  color="black")
