...
def panel_recovery(ax, camp_sigs):
    ...
    worst_truth = np.minimum(camp_sigs["campA_cos_truth"], camp_sigs["campB_cos_truth"])
    ax.vlines(x, 0.97, camp_sigs["between_camp_cos"], color="#cccccc", lw=1)
    ax.scatter(
        x,
        camp_sigs["between_camp_cos"],
        s=42,
        color=PALETTE["stiff"],
        zorder=3,
        label="between camps (A vs B)",
    )
    ax.scatter(
        x,
        worst_truth,
        ...
        color=PALETTE["soft"],
        zorder=3,
        label="camp vs truth (worst of A,B)",
    )
...
def panel_loglik(ax, split):
    ...
    ax.hist(
        g[g >= 0], bins=bins, color=PALETTE["stiff"], alpha=0.85, label="favours camp A"
    )
    ax.hist(g[g < 0], bins=bins, color="#b2182b", alpha=0.85, label="favours camp B")
...
def build_figure(df, camp_sigs, sigma_min, sigma_max, split_threshold, rho):
    ...
    if camp_sigs is not None:
        ...
        ax[0, 0].text(
            0.5,
            0.5,
            "camp_signatures.csv not supplied",
            ha="center",
            va="center",
            color=PALETTE["grey"],
        )
...
def main():
    ...
    ap.add_argument(
        "--split-threshold", type=float, default=0.05, dest="split_threshold"
    )
    ap.add_argument(
        "--rho", default=None, help="Correlation label for the title (optional)."
    )
    ...
    camp_sigs = (
        pd.read_csv(args.camp_sigs)
        if args.camp_sigs and Path(args.camp_sigs).exists()
        else None
    )
    sigma_min, sigma_max = _resolve_sigmas(args)

    fig = build_figure(
        df, camp_sigs, sigma_min, sigma_max, args.split_threshold, args.rho
    )
    save(fig, args.outdir, args.name)
    print(
        f"Written: {Path(args.outdir) / (args.name + '.pdf')}, "
