...
def _camp_best_draw(post, obj, model, logp_fn, chains, thin, fwd_fns):
    ...
    for c in chains:
        for dr in range(0, post.sizes["draw"], thin):
            constrained = {
                rv.name: post[rv.name].isel(chain=c, draw=dr).values
                for rv in model.free_RVs
            }
...
def main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=12,
        help="power-iteration steps per point, warm-started; the first point uses more",
    )
    ...
    print(
        f"camp A rep chain {cA} draw {dA} logp {lpA:.1f}; "
    ...
    for t in ts:
        ...
        act_t = {
            n: np.clip(x, 1e-9, None) / np.clip(x, 1e-9, None).sum()
    ...
    sampler_barrier = (
        float(ll_end_s - inside.loglik_sampler.min()) if len(inside) else float("nan")
    )
    simplex_barrier = (
        float(ll_end - inside.loglik.min()) if len(inside) else float("nan")
    )
    on_path_med = float(np.nanmedian(np.abs(df.on_path_curv)))
    stiff = (
        float(-np.nanmin(df.orth_curv_min))  # magnitude of stiffest wall
        if df.orth_curv_min.notna().any()
        else float("nan")
    )
        ...
        [
            {
                ...
                "simplex_barrier": simplex_barrier,
                "sampler_barrier": sampler_barrier,
                "logit_inflation": sampler_barrier - simplex_barrier,
                "on_path_curv_med": on_path_med,
                "orth_curv_stiffest": -stiff,
                "condition_number": condition,
                "adapted_step": adapted_step,
                "implied_step": implied_step,
                "tree_depth": tree_depth,
                "steps_to_cross": steps_to_cross,
                "max_trajectory_steps": max_traj,
    ...
    print(
        f"simplex vs sampler loglik barrier : {simplex_barrier:.1f} / {sampler_barrier:.1f} "
        f"(logit inflation {sampler_barrier - simplex_barrier:+.1f})"
    )
    print(f"on-path |curv| (median)           : {on_path_med:.3g}")
    print(
        f"stiffest orthogonal curvature     : {-stiff:.3g}  (condition {condition:.0f})"
    )
    print(
        f"stiffest exact curvature (power)  : {stiff_exact_signed:.3g}  "
        f"(condition {condition_exact:.0f}, implied step {implied_exact:.4g})"
    )
    print(
        f"adapted step {adapted_step:.4g} vs implied 2/sqrt(stiff) {implied_step:.4g}"
    )
    print(
        f"steps to cross {steps_to_cross:.0f} vs max trajectory 2^depth {max_traj:.0f}"
    )
    print(
        f"non-finite points {(~df.finite).sum()} / {len(df)}; "
