...
def panel_activity(ax, df):
    ...
    ax.set_ylim(0, 2.02)
    _bands(ax)
    ax.plot(r, df["L1_best_median"], "-o", color=PALETTE["stiff"], lw=2, ms=7, zorder=3)
    ax.axhline(0.1, color=PALETTE["grey"], ls=":", lw=1)
    ax.text(0.05, 0.13, "close threshold 0.1", fontsize=7.8, color=PALETTE["grey"])
...
def panel_conditioning(ax, df):
    ...
    r = df["correlation"].values
    ax.plot(r, df["smallest_sv"], "-o", color=PALETTE["accent"], lw=2, ms=7, zorder=3)
...
def panel_trap(ax, df):
    ...
    ax2.plot(r, df["eff_num_sigs"], "-D", color=PALETTE["soft"], lw=1.8, ms=6, zorder=4)
    ...
    ax.annotate(
        "ess best but collapsed to ~1 signature",
        (worst["correlation"], worst["ess_median"]),
        (0.55, worst["ess_median"] * 0.82),
        fontsize=7.8,
        color="#333333",
...
def build_figure(df):
    ...
    fig.suptitle(
        "De novo signature discovery across the signature-overlap sweep",
        fontsize=13,
        fontweight="bold",
        y=0.985,
    )
...
def main():
    ...
    ap.add_argument(
        "--sweep", required=True, help="sweep_master.csv from sweep_aggregate.py."
    )
