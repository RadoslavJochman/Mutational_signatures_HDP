...
def main():
    ...
    if a.true_signatures is not None and "signatures" not in post.data_vars:
        print(
            "note: --true-signatures given but the trace has no `signatures` "
    ...
    sig_pc = (
        [
            signature_distances(Smean[c][perms[c]], true_S, metrics)
    ...
    expo_pc = np.stack(
        [exposure_errors(A[:, c, :], true_acts) for c in range(n_chains)], axis=1
    )  # (K, chains)
    ...
    for m in metrics:
        if do_sigs:
            ...
            sig_tbl[f"sig_{m}_best"] = b
            sig_tbl[f"sig_{m}_mean"] = mn
            sig_tbl[f"sig_{m}_worst"] = w
        b, mn, w = across_chain(use_st[m], m)
        sig_tbl[f"use_{m}_best"] = b
        sig_tbl[f"use_{m}_mean"] = mn
    ...
    for m in metrics:
        ...
        node_tbl[f"act_{m}_best"] = b
        node_tbl[f"act_{m}_mean"] = mn
    ...
    try:
        ...
        fig.tight_layout()
        fig.savefig(out / "recovery_vs_truth.png", dpi=150)
